...
NAME_EXCLUDE_KEYWORDS = ["중고", "노트북", "리퍼", "전시상품", "해외구매", "병행수입", "탈거"]
OPTION_EXCLUDE_KEYWORDS = ["중고", "리퍼", "전시", "해외구매", "병행수입", "탈거"]

# 개인 PC 견적용이 아닌(서버/데이터센터/워크스테이션) VGA 제외 키워드
# 이름 기반 안전장치 1차 - 신규 모델명은 계속 추가 필요 (2차 안전장치는 아래 VGA_MAX_PRICE 가격 상한선)
VGA_SERVER_EXCLUDE_KEYWORDS = [
    "A100", "A800", "A30", "A40",           # NVIDIA Ampere 데이터센터
    "H100", "H200", "H800",                  # NVIDIA Hopper 데이터센터
    "B100", "B200",                          # NVIDIA Blackwell 데이터센터
    "L40", "L4 ",                            # NVIDIA Ada 데이터센터/추론용
    "RTX PRO", "Quadro", "Tesla",            # 워크스테이션/구 데이터센터 라인
    "NVL", "SXM",                            # 데이터센터 전용 폼팩터
    "워크스테이션",
]
# 소비자용 최상위 게이밍 GPU도 이 가격은 넘지 않는다는 전제의 안전장치(원)
VGA_MAX_PRICE = 5000000

# 개인 PC 견적용이 아닌(서버/ECC) RAM 제외 기준
# 옵션명에서 파싱한 총 용량(GB)이 이 값을 초과하면 제외 (소비자용 데스크탑 RAM은 보통 스틱당 48GB 이하)
RAM_MAX_CAPACITY_GB = 64

# 개인 PC 견적용이 아닌(서버) RAM 제외 기준
RAM_SERVER_EXCLUDE_KEYWORDS = ["서버용", "ECC", "레지스터드", "RDIMM", "LRDIMM", "Registered"]

# CPU: 인텔 서버용(제온) 제외 - AMD는 이미 칩 필터에서 통째로 제외되므로 인텔 서버 라인업만 추가
CPU_SERVER_EXCLUDE_KEYWORDS = ["제온", "Xeon"]

# MBoard: 서버용 칩셋/폼팩터 제외
MBOARD_SERVER_EXCLUDE_KEYWORDS = ["C621", "C622", "SP3", "SP5", "서버용", "서버보드"]

# Power: 서버용(이중화/리던던트) 제외
POWER_SERVER_EXCLUDE_KEYWORDS = ["리던던트", "redundant", "Redundant", "서버용", "CRPS"]

# Case: 서버/랙마운트용 제외
# 주의: 'rack'을 단독으로 쓰면 'Bracket'(브래킷) 같은 정상 상품명에 오탐되므로
#       반드시 '랙마운트'처럼 구체적인 한글 표기만 사용할 것
CASE_SERVER_EXCLUDE_KEYWORDS = ["랙마운트", "서버케이스", "랙케이스", "19인치 랙"]
...
BRAND_INCLUDE_KEYWORDS = {
    "mboard": ["MSI", "ASUS", "에이수스", "GIGABYTE", "기가바이트"],
    "cooler": ["PCCOOLER", "피씨쿨러", "DEEPCOOL", "딥쿨"],
    "ram":    ["삼성전자", "TeamGroup", "팀그룹"],
    "ssd":    ["삼성전자"],
    "hdd":    ["Western Digital", "WD ", "웬디", "Seagate", "씨게이트"],
    "power":  ["마이크로닉스", "darkFlash", "다크플래시"],
...
def build_category_sql(label, csv_file, prefix):
    ...
    is_cpu = (prefix == "cpu")
    is_power = (prefix == "power")
    is_case = (prefix == "case")

    # 개인 PC 견적용이 아닌(서버/데이터센터/워크스테이션/ECC) 상품은 카테고리별로
    # 이름 제외 키워드를 추가해서 함께 걸러냄 (기본 NAME_EXCLUDE_KEYWORDS + 카테고리 전용)
    name_exclude_keywords = list(NAME_EXCLUDE_KEYWORDS)
    if is_vga:
        name_exclude_keywords += VGA_SERVER_EXCLUDE_KEYWORDS
    if is_ram:
        name_exclude_keywords += RAM_SERVER_EXCLUDE_KEYWORDS
    if is_cpu:
        name_exclude_keywords += CPU_SERVER_EXCLUDE_KEYWORDS
    if is_mboard:
        name_exclude_keywords += MBOARD_SERVER_EXCLUDE_KEYWORDS
    if is_power:
        name_exclude_keywords += POWER_SERVER_EXCLUDE_KEYWORDS
    if is_case:
        name_exclude_keywords += CASE_SERVER_EXCLUDE_KEYWORDS
...
-- RAM 전용: 옵션명 맨 앞 숫자(총 용량 GB)가 {RAM_MAX_CAPACITY_GB}GB 초과인 옵션 제외
-- (묶음 개수가 1개라도 단일 스틱 자체가 대용량인 서버/ECC RAM을 걸러내기 위함)
-- 예) option_name = '64GB_67,526원/1GB' -> 앞의 '64'만 추출해서 비교
DELETE FROM {prefix}_prices_all
WHERE CAST(REGEXP_SUBSTR(option_name, '^[0-9]+') AS UNSIGNED) > {RAM_MAX_CAPACITY_GB};
""" if is_ram else ""

    vga_price_filter = f"""
-- VGA 전용: 개인 PC 견적용이 아닌 초고가(데이터센터/워크스테이션) 옵션 제외 (안전장치)
-- 이름 키워드(VGA_SERVER_EXCLUDE_KEYWORDS)로 못 걸러낸 신규/누락 모델 대비용
DELETE FROM {prefix}_prices_all
WHERE price > {VGA_MAX_PRICE};
""" if is_vga else ""
...
DELETE FROM {prefix}_prices_all
WHERE {opt_excl};
{ram_stick_filter}{vga_price_filter}
-- 6) 최종 적재 대상 상품 판별
